...
Existing frameworks may be used for further analysis -- an interface to \pkg{coda} \citep{plummer2006} for checking outputs, analysis, and diagnostics is provided. The \pkg{BVARverse} \citep{vashold2020} companion package allows integration into a \pkg{tidyverse} \citep{wickham2019} oriented workflow and facilitates flexible plotting with \pkg{ggplot2} \citep{wickham2016}.
