...
%   - remove nojss from documentclass options
%   - increase n_draw to 50000
%   - increase n_burn to 25000
%   - increase n_cores to 4
...
<<timeseries, fig=TRUE, eval=TRUE, echo=FALSE, include=FALSE, width=8, height=5>>=
...
% Hardcoded to match outputs above % 2020-04-27
\begin{Soutput}
  Bayesian VAR consisting of 238 observations, 6 variables and 5 lags.
  Time spent calculating: 2.74 mins
...
<<trace_density, fig=TRUE, png=TRUE, eval=TRUE, echo=TRUE, include=FALSE>>=
plot(run)
@
...
<<betas, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=8, height=4>>=
...
\begin{figure}[!ht]
	\centering
  \includegraphics[width=0.9\textwidth]{fig-betas.pdf}
...
@
<<irf_cholesky, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE>>=
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.9\textwidth]{fig-irf_cholesky.pdf}
...
Forecast objects feature methods for plotting, printing, and summarizing. The \code{plot()} method can be used to subset plots to certain variables via the \code{vars} argument. The visualization can include a number of realized values before the forecast, which is set with the argument \code{t_back} (see Figure~\ref{fig:predict_unconditional}).

<<predict, eval=TRUE, echo=TRUE>>=
predict(run) <- predict(run, horizon = 16, conf_bands = c(0.05, 0.16))
@
<<predict_unconditional, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE>>=
...
<<app_timeseries, fig=TRUE, eval=TRUE, echo=FALSE, include=FALSE, width=8, height=2.8>>=
...
stopCluster(cl)
@
<<app_coda_chains, eval=TRUE, echo=TRUE>>=
runs_mcmc <- as.mcmc(runs, vars = "lambda")
gelman.diag(runs_mcmc, autoburnin = FALSE)
@
<<app_chains, fig=TRUE, png=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=8, height=2.8>>=
...
@
<<app_irf_signs, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE>>=
plot(irf(run_app))
@
...
@
<<app_predict_conditional, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=8, height=5>>=
