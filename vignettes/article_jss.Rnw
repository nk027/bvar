\documentclass[article]{jss} % add nojss for vignette
...
\begin{Soutput}
  ...
  Time spent calculating: 50 secs
...
<<betas, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=6, height=2.4>>=
plot(run, type = "dens", mfrow = c(1, 2),
  vars_response = "GDPC1", vars_impulse = c("GDPC1-lag1", "FEDFUNDS-lag2"))
@
...
<<residuals, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=6, height=3.6>>=
...
@
<<irf_cholesky, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=6, height=6>>=
...
@
<<predict_unconditional, fig=TRUE, eval=TRUE, echo=TRUE, include=FALSE, width=6, height=6>>=
